...
* スロット数（$N = 3, 4, 5, 7$ 等）が与えられた際、プレイヤーの視野角（約 $145^\circ$ 以内）に収めつつ、下端隙間（$\ge 8.0\,\text{cm}$）を数学的に担保する最適幾何計算関数 `CreateOptimized(int count)` を実装：
  - $N \le 3$: 半径 $R = 1.30\,\text{m}$、展開角約 $39^\circ$（全体展開角 $78^\circ$）➔ コンパクトな視界
  - $N \le 5$: 半径 $R = 1.40\,\text{m}$、展開角約 $36^\circ$（全体展開角 $144^\circ$）➔ 視界いっぱいの標準コックピット
  - $N \le 7$: 半径 $R = 1.65\,\text{m}$、展開角約 $29^\circ$（全体展開角 $174^\circ$）➔ 背中に回らず視界内に収まるワイド展開
* これにより、ユーザーおよび外部ゲームルールは「手札枚数」を指定するだけで、一切の幾何パラメータを手動計算することなく完璧な円弧空間を一発生成できる。
