...
## 30. ポータブル4大AIエージェント体制（Architect, Coder, Refactorer, UdonInspector）の運用数理

### ① 単一知能から専門分業知能へのシフト
* 1つのAIプロンプトに「ユーザーの意図を汲む」「仕様を設計する」「コードを書く」「抽象モデルへ整流化する」「U#制約をチェックする」のすべてを同時に課すと、タスク間のトレードオフ（動かすことを優先して設計原則をショートカットするなど）により品質低下が発生しやすい。
* 専門の役割を分離し、直列リレー形式（**01. Architect ➔ 02. Coder ➔ 03. Refactorer ➔ 04. UdonInspector**）で検証パイプラインを回すことで、各役割の認知負荷を最小化し、ゼロ妥協の高品質コードを保証する。
* 特にコード修正を行う役（Coder ＆ Refactorer）の直後に最終門番として「04. UdonInspector（変更不可の監査官）」を配置することで、リファクタリング後のコードがU#言語制限や同期作法をすり抜ける穴を構造的に封鎖する。
...
### ③ YAGNI原則に基づく場の状態判定シンプル化
*   **課題の反省**: ゾーン種別の抽象化を進めるにあたり、シリアライズ値やデフォルト値の齟齬による過剰な3層防護（Start()での強制自己修復等）を企図しかけたが、憲章 5.1「YAGNI原則 ＆ 不要な堅牢化の禁止」に立ち戻り設計を洗練。
*   **シンプルで明快な処理**:
    *   カードの現在ゾーン（`card.GetCurrentZone()`）が共通の場（`centerPlayZone`）であれば、**単に即座に `return` し、Debugログに「中央プレイエリア（場）にあるカードは操作できません」を出力する** のみに集約。
    *   手札スロットにあるカードであれば各モードの手札挙動（即時プレイ / 複数選択トグル）を実行する。
    *   これにより、余計なコードを追加することなく、シングルモード・マルチモード双方で場のカードへの誤操作を完璧に抑止した。

---

## 44. KISS/YAGNI原則の徹底と不必要なスコープ・デッドコード・過剰堅牢化の排除

### ① 先回り抽象化の死にコード化（Rule of Three違反）の反省
*   **事象**: 「手札」と「場」の2状態しかない段階で、将来の拡張（墓地、山札等）を先回りして `CardZoneType` Enum や判定ヘルパー（`IsInHand()`, `IsOnField()`）を新設した。
*   **結果**: 実際の制御ロジックでは `zone == centerPlayZone` で直接判定した方がシンプルかつ直感的であり、新設したEnumやメソッドは一度も実務で使われない「死にモデル」と化していた。
*   **教訓**: 3回以上同じパターンが出現するまで抽象化を行わない（Rule of Three）。今必要な最小限の比較（`zone == centerPlayZone`）に絞ることで、可読性と保守性が大幅に向上する。

### ② 多重防護・二重ガードの排除
*   **事象**: 門番メソッド（`OnCardClicked`）で既に「場にあるカードは警告を出して即 `return`」しているにもかかわらず、後続の内部メソッド（`ToggleCardSelection`）でも再度 `zone == centerPlayZone` を判定していた。
*   **教訓**: 呼び出し順序が確定している内部処理にまで「念のため」と多重にガード節を設けると、コードが冗長化し、どちらが真の境界条件なのか分からなくなる。内部処理は呼び出し元を信頼し、二重防護を排除する。

### ③ 不用意なスコープ公開の解消（カプセル化と[SerializeField] privateの統一）
*   **メソッドのスコープ狭小化**: 外部から呼ばれることのない内部専用メソッド（`PlayCardImmediate`, `ToggleCardSelection`, `JoinSeat`, `LeaveSeat`, `SnapTo` 等）が安易に `public` になっていたため、すべて `private` に降格した。
*   **フィールドのカプセル化**: Inspector設定用フィールドを手抜きで `public` に直書きしていた箇所（`playMode`, `centerPlayZone`, `cardPool`, `snapZones`, `deckManager` 等）をすべて `[SerializeField] private` に統一し、必要なアクセサ（プロパティ / セッター）を整備した。
*   **内部配列の保護**: 固定長バッファ配列をそのまま返していたデッドGetter（`GetSelectedOrder()`, `GetStackedCards()`）を削除し、内部状態の意図せぬ改ざんを防止した。
*   **動的自己修復コードの撤廃**: `GetComponentInChildren` 等による「参照が外れていたら自動で拾う」コードは、Inspector・ビルダーによる明示的バインド原則を曖昧にし、思わぬバグの温床となるため完全撤廃した。

---

## 45. ソフトウェア開発原則の再体系化と優先順位の定式化（SOLID / KISS / YAGNI / DRY）

### ① 開発原則間の衝突と優先順位の確立
オブジェクト指向開発において広く知られる SOLID, KISS, YAGNI, DRY 原則は、時に互いにトレードオフの関係（衝突）に陥る。
例えば、「将来の変更に備えて抽象化・分割する（OCP/SRP）」姿勢は、「今必要なものだけに留め、シンプルに保つ（YAGNI/KISS）」姿勢と真っ向から衝突し、過剰なEnum・汎用クラス・自己修復コードといった「オーバーエンジニアリング」を誘発する。

本プロジェクトではこの衝突を明確に調停するため、以下の優先不等式をプロジェクト憲章（AGENTS.md）に明文化した：

$$\mathbf{KISS \;\ge\; YAGNI \;>\; DRY \;>\; SRP / ISP \;>\; OCP（先回り抽象化）}$$

*   **KISS原則（最上位）**: 最もシンプルで直接的な実装（例: `zone == centerPlayZone`）を尊ぶ。
*   **YAGNI ＆ 3倍則 (Rule of Three)**: 同一の概念・分岐が3箇所以上重複するまで抽象化（Enum昇格やインターフェース化）を禁止する。
*   **DRY ＆ SRP / ISP**: 単一責任とインターフェース分離（カプセル化）を徹底し、内部実装は `private`、Inspector設定は `[SerializeField] private` に集約する。

### ② 01. Architect AI による Plan 策定時の原則監査フロー
AIが自律的に開発を進める中で、プロンプトの指示を過剰に解釈して不要な堅牢化や過剰設計に走る現象（Gemini/LLMの特性）を恒久的に防ぐため、以下の分業体制を確立した：
1.  **Plan策定時における事前監査**:
    *   Antigravityがタスクの実行Planを作成する際、システムプロンプト（`agents/01_architect.md`）に基づき、要件がKISS/YAGNI/3倍則に違反していないかを事前監査する。
2.  **原則矛盾の検知とエスカレーション（User Review Required）**:
...
## 29. 手札の論理配列管理と物理スロットページングスクロールの設計数理（T52）

### ① 物理スロット固定と大量手札（ゴーアウト系ゲーム）のエルゴノミクス矛盾
*   **直面した課題**:
    *   プレイヤー包囲型円弧スロット（Arcade Cockpit Layout）は、視認性と操作性を担保するためにスロット数5〜7個がエルゴノミクス上の上限である（これ以上増やすと視界を遮り腕が届かなくなる）。
    *   しかし、大富豪（4人時1人13〜14枚）や七並べ、UNO等のゲームでは、初期ディールで10枚以上のカードを保持する必要があり、「物理スロット数＝手札保持上限」という初期設計ではスロット溢れ（ドロー拒絶エラー）が発生していた。
*   **解決のアーキテクチャ（論理手札と物理表示の完全分離）**:
    *   `PersonalHandArea.cs` にて、手札の真実源（SSOT）を固定長配列 `CardController[] heldCards`（最大64枚）として内部保持。
    *   物理スロット（`CardSnapZone[] snapZones`、通常5枠）は、単なる**「現在ページのビューポート（窓）」**として機能させる。
    *   手元操作パネルに「◀ 前へ」「次へ ▶」ボタン（`HandScrollButton.cs`）を新設し、ページオフセット（`pageOffset`）単位で物理スロットへ表示カードを瞬時にスワップ（非表示カードは `gameObject.SetActive(false)` による完全退避）するページング機構を確立。

### ② ページング数理と手札枚数要約アルゴリズム
*   有効物理スロット数を $S = \text{GetActiveSlotCount()}$（通常5）、現在の総手札枚数を $N$（$0 \le N \le 64$）、ページオフセットを $P \in \{0, S, 2S, \dots\}$ とする。
*   各物理スロット $i \in [0, S-1]$ に配置されるカードの論理インデックスは：
    $$k = P + i$$
    *   $k < N$ の場合：`heldCards[k]` をスロットへ吸着表示（`SetActive(true)` ＆ `TrySnap`）。
    *   $k \ge N$ の場合：スロットを解放・空スロット化。
*   **手元TMP表示フォーマット**:
    *   $N = 0$: `手札: 0枚`
    *   $1 \le N \le S$: `手札: N枚`（全枚数が表示されているため範囲表記を省略してすっきり表示）
    *   $N > S$: `手札: N枚 (P+1 〜 \min(P+S, N))`（例: `手札: 13枚 (1〜5 / 13)`）
*   **動的ページ自動補正**:
    *   カードプレイ（`RemoveCard`）によって現在のページが表示外（空）になった場合、自動的に $P \leftarrow \max(0, P - S)$ へ前倒し補正し、プレイヤーが常に手札を直視できる防護トランザクションを実装。

### ③ TMP文字化け（JIS第1水準と特殊幾何学記号の境界）とUI階層設計
*   **Missing Glyph（■表示）の根本原因**:
    *   日本語TMPフォントアセット（`NotoSansJP-Medium SDF.asset`）生成時に一般的な「JIS第1水準漢字＋ひらがな・カタカナ＋ASCII」を指定した場合、「前」「次」等の漢字は収録されるが、特殊記号 **`◀`** (U+25C0) や **`▶`** (U+25B6) は収録範囲外（特殊幾何学記号）となる。
    *   このため、ボタン等で `< 前へ` や `次へ >` のようにASCIIの `<` `>` を用いるか、純粋な日本語表記にするのが堅牢な防護策となる。波ダッシュ `〜` も同様にハイフン `-` に代替するのが安全である。
*   **3段UIエルゴノミクス（VRパーソナル操作盤）**:
    *   最上段（Y: +0.17m）: 手札枚数要約（`HandCount_Display`）
    *   中段（Y: +0.10m）: スクロール操作（`< 前へ` / `次へ >`）
    *   下段（Y: 0.00m）: 主アクション（`引く` / `出す`）
    *   各段に上下 1.5cm〜2cm の安全マージンを設けることで、VRコントローラーによる誤タップを物理的に防止。

### ④ 今後のUI最適化課題（T52未完タスクの設計検討）
1.  **スクロール単位の最適化（1枚送り vs 5枠一括送り）**:
    *   現状の枠数一括送り（1〜5 ➔ 6〜10）は移動量が大きい反面、隣接カード（5枚目と6枚目）を同時に見てペア選択することが困難。
    *   左右1枚シフト（`pageOffset++` / `pageOffset--`）にすることで、手札スライダーのようにシームレスなカード選別が可能になるため、今後の改修候補とする。
2.  **手札枚数表示の視認性向上（背景プレート・コントラスト確保）**:
    *   背景なしのゴールド色文字のみでは、ワールド内のテーブル木目やカードオブジェクトの色調と同化して埋没しやすい。
    *   文字背面に半透明・濃色のバックプレート（薄型Quad/Cube）を配置し、視認性を向上させる調整を検討する。

### ⑤ 品質検証結果
*   Test Project内の `Assembly-CSharp.csproj`（Runtime / U#）および `Assembly-CSharp-Editor.csproj`（Editor / Builder）の双方に対し、`dotnet build` による厳格な事前コンパイル検証を実施し、**0 エラー** を確認。
